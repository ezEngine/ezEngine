...
struct EZ_EDITORFRAMEWORK_DLL ezPluginInfo
{
  ...
  {
    return m_bAvailable == rhs.m_bAvailable && m_bActive == rhs.m_bActive && m_bToBeLoaded == rhs.m_bToBeLoaded && m_bLoadCopy == rhs.m_bLoadCopy;
  }
  bool operator!=(const ezPluginInfo& rhs) const { return !(*this == rhs); }
};

/// \brief Describes a set of plugins. Name is the plugin file name used to load it through ezPlugin, ezPluginInfo contains details about the loading
/// state.
