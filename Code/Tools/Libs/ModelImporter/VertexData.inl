...
{
  inline bool VertexDataStream::HasValue(VertexIndex index) const { return m_IndexToData.GetCount() > index && m_IndexToData[index].IsValid(); }
  ...
  inline void VertexDataStream::AddValues(const ezArrayPtr<ezUInt8>& values)
  {
  ...
  {
    EZ_ASSERT_DEV(sizeof(Attribute) % this->m_DataStream.GetAttributeSize() == 0, "Size of attribute type ({0}) must equal the attribute size of the wrapped vertex stream ({1})", (ezUInt32)sizeof(Attribute), this->m_DataStream.GetAttributeSize());
    ...
    static_assert(std::is_same<float, Attribute>::value || std::is_same<ezInt32, Attribute>::value || std::is_same<ezUInt32, Attribute>::value || std::is_same<ezVec2, Attribute>::value || std::is_same<ezVec3, Attribute>::value || std::is_same<ezVec4, Attribute>::value || std::is_same<ezVec2I32, Attribute>::value || std::is_same<ezVec3I32, Attribute>::value || std::is_same<ezVec4I32, Attribute>::value || std::is_same<ezVec2U32, Attribute>::value || std::is_same<ezVec3U32, Attribute>::value || std::is_same<ezVec4U32, Attribute>::value, "Invalid attribute type.");

#define STATIC_VEC_CHECK(ELEM_COUNT, TYPE, TYPENAME)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
  else if (std::is_same<ezVec##ELEM_COUNT##Template<TYPE>, Attribute>::value) { EZ_ASSERT_DEV(sourceDataStream.GetElementType() == VertexElementType::TYPENAME && sourceDataStream.GetNumElementsPerVertex() == ELEM_COUNT, "Vertex data stream is incompatible with the given vector type!"); }

    if (std::is_same<float, Attribute>::value)
    {
      EZ_ASSERT_DEV(sourceDataStream.GetElementType() == VertexElementType::FLOAT && sourceDataStream.GetNumElementsPerVertex() == 1, "Vertex data stream is incompatible with float attributes!");
    }
    else if (std::is_same<ezInt32, Attribute>::value)
    {
      EZ_ASSERT_DEV(sourceDataStream.GetElementType() == VertexElementType::INT32 && sourceDataStream.GetNumElementsPerVertex() == 1, "Vertex data stream is incompatible with ezInt32 attributes!");
    }
    else if (std::is_same<ezUInt32, Attribute>::value)
    {
      EZ_ASSERT_DEV(sourceDataStream.GetElementType() == VertexElementType::UINT32 && sourceDataStream.GetNumElementsPerVertex() == 1, "Vertex data stream is incompatible with ezUInt32 attributes!");
    }
...
#undef VEC_CHECK
  ...
  {
    const ezUInt8* data = reinterpret_cast<const ezUInt8*>(&value);
  ...
  {
    const ezUInt8* data = reinterpret_cast<const ezUInt8*>(values.GetPtr());
