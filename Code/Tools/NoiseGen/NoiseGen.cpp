...
ezVec3 hash33(ezVec3 p, ezUInt32 seed)
{
...
float gradientNoise(ezVec3 x, float freq, ezUInt32 seed)
{
  // grid
  ezVec3 p(floor(x));
  ezVec3 w(frac(x));
  ...
  // gradients
  ezVec3 ga = hash33((p + ezVec3(0.0f, 0.0f, 0.0f)).Mod(freq), seed);
  ezVec3 gb = hash33((p + ezVec3(1.0f, 0.0f, 0.0f)).Mod(freq), seed);
  ezVec3 gc = hash33((p + ezVec3(0.0f, 1.0f, 0.0f)).Mod(freq), seed);
  ezVec3 gd = hash33((p + ezVec3(1.0f, 1.0f, 0.0f)).Mod(freq), seed);
  ezVec3 ge = hash33((p + ezVec3(0.0f, 0.0f, 1.0f)).Mod(freq), seed);
  ezVec3 gf = hash33((p + ezVec3(1.0f, 0.0f, 1.0f)).Mod(freq), seed);
  ezVec3 gg = hash33((p + ezVec3(0.0f, 1.0f, 1.0f)).Mod(freq), seed);
  ezVec3 gh = hash33((p + ezVec3(1.0f, 1.0f, 1.0f)).Mod(freq), seed);
...
float worleyNoise(ezVec3 uv, float freq, ezUInt32 seed)
{
  ezVec3 id(floor(uv));
  ezVec3 p(frac(uv));
  ...
  for (float x = -1.f; x <= 1.0f; x += 1.0f)
  {
    for (float y = -1.0f; y <= 1.0f; y += 1.0f)
    {
      for (float z = -1.0f; z <= 1.0f; z += 1.0f)
      {
        ...
        ezVec3 h = hash33(mod2(id + offset, freq), seed) * 0.5f + ezVec3(0.5f);
        h += offset;
        ezVec3 d = p - h;
...
float worleyFbm(ezVec3 p, float freq, ezUInt32 seed)
{
    ...
    for (uint32_t slice = 0; slice < 128; slice++)
    {
      ...
      for (uint32_t y = 0; y < 128; y++)
      {
        ...
        for (uint32_t x = 0; x < 128; x++, curPixel++)
        {
          ...
          float perlinFbm = ezMath::Lerp(1.0f, perlinfbm(uvw, frequency, 7, seed), 0.5f);
