...
namespace ezMeshImportUtils
{
  ...
  //    pAccessor->SetValue(pMaterialProperties, "BaseTexture", ezVariant(ezMeshImportUtils::ImportOrResolveTexture(szImportSourceFolder, szImportTargetFolder, baseTexture->m_FileName, ezModelImporter2::TextureSemantic::DiffuseMap, false))).LogFailure();
  //  }
  //  else
  //  {
  ...
  //    if (normalTexture)
  //      pAccessor->SetValue(pMaterialProperties, "NormalTexture", ezVariant(ezMeshImportUtils::ImportOrResolveTexture(szImportSourceFolder, szImportTargetFolder, normalTexture->m_FileName, ezModelImporter2::TextureSemantic::NormalMap, false))).LogFailure();
  //    else
  //    {
  ...
  //    }
  //    if (roughnessTexture)
  //      pAccessor->SetValue(pMaterialProperties, "RoughnessTexture", ezVariant(ezMeshImportUtils::ImportOrResolveTexture(szImportSourceFolder, szImportTargetFolder, roughnessTexture->m_FileName, ezModelImporter2::TextureSemantic::RoughnessMap, false))).LogFailure();
  //    else
  ...
  //    pAccessor->SetValue(pMaterialProperties, "MetallicTexture", ezVariant(ezMeshImportUtils::ImportOrResolveTexture(szImportSourceFolder, szImportTargetFolder, metalTexture->m_FileName, ezModelImporter2::TextureSemantic::MetallicMap, false))).LogFailure();
  //  }
  //  else
  //  {
  ...
  static void ImportMeshAssetMaterialProperties(ezMaterialAssetDocument* pMaterialDoc, const ezModelImporter2::OutputMaterial& material, const char* szImportSourceFolder, const char* szImportTargetFolder)
  {
    ...
    // Set Normal Texture / Roughness Texture
    {
      ezString textureNormal, textureRoughness;
      ...
      if (!textureNormal.IsEmpty() || !textureRoughness.IsEmpty())
      {
        pAccessor->SetValue(pMaterialProperties, "UseNormalAndRoughnessTexture", true).LogFailure();
    ...
    {
      ezString textureMetallic;

      if (!material.m_TextureReferences.TryGetValue(ezModelImporter2::TextureSemantic::MetallicMap, textureMetallic))
      {
        // Due to the lack of options in stuff like obj files, people stuff metallic into the ambient slot.
        material.m_TextureReferences.TryGetValue(ezModelImporter2::TextureSemantic::AmbientMap, textureMetallic);
      }
    ...
    // Set emissive texture
    {
      ...
      {
        pAccessor->SetValue(pMaterialProperties, "UseEmissiveTexture", false).LogFailure();
      }
    }
