...
ezQtDLangAssetDocumentWindow::ezQtDLangAssetDocumentWindow(ezAssetDocument* pDocument)
  : ezQtDocumentWindow(pDocument)
{
  // Menu Bar
  {
    ...
    context.m_sMapping = "DLangAssetMenuBar";
  ...
  // Tool Bar
  {
    ...
    ezActionContext context;
    context.m_sMapping = "DLangAssetToolBar";
    context.m_pDocument = pDocument;
    context.m_pWindow = this;
    pToolBar->SetActionContext(context);
    pToolBar->setObjectName("DLangAssetWindowToolBar");
    addToolBar(pToolBar);
  }
  ...
  // Property Grid
  {
    ...
    pPropertyPanel->setObjectName("DLangAssetDockWidget");
    pPropertyPanel->setWindowTitle("DLang Properties");
    pPropertyPanel->show();
  ...
  m_pAssetDoc = static_cast<ezDLangAssetDocument*>(pDocument);
  ...
  // central widget
  {
    ...
    m_pHighlighter = new DLangHighlighter(m_pSourceLabel->document());
  ...
  m_pAssetDoc->GetEvent().AddEventHandler(ezMakeDelegate(&ezQtDLangAssetDocumentWindow::DocumentEventHandler, this));
}

ezQtDLangAssetDocumentWindow::~ezQtDLangAssetDocumentWindow()
{
  m_pAssetDoc->GetEvent().RemoveEventHandler(ezMakeDelegate(&ezQtDLangAssetDocumentWindow::DocumentEventHandler, this));

  delete m_pHighlighter;
}

void ezQtDLangAssetDocumentWindow::UpdateFileContentDisplay()
{
...
void ezQtDLangAssetDocumentWindow::DocumentEventHandler(const ezDLangAssetDocumentEvent& e)
{
  switch (e.m_Type)
  {
    case ezDLangAssetDocumentEvent::Type::ScriptCreated:
    case ezDLangAssetDocumentEvent::Type::ScriptOpened:
    case ezDLangAssetDocumentEvent::Type::ScriptTransformed:
      UpdateFileContentDisplay();
      break;
...
DLangHighlighter::DLangHighlighter(QTextDocument* parent)
  : QSyntaxHighlighter(parent)
{
  // default color scheme
  m_colors[DLangEdit::Comment] = QColor("#6A8A35");
  m_colors[DLangEdit::Number] = QColor("#B5CEA8");
  m_colors[DLangEdit::String] = QColor("#CE916A");
  m_colors[DLangEdit::Operator] = QColor("#808000");
  m_colors[DLangEdit::KeywordBlue] = QColor("#569CCA");
  m_colors[DLangEdit::KeywordPink] = QColor("#C586C0");
  m_colors[DLangEdit::KeywordGreen] = QColor("#4EC9B0");
  m_colors[DLangEdit::BuiltIn] = QColor("#DCDCAA");
...
void DLangHighlighter::highlightBlock(const QString& text)
{
  // parsing state
  enum
  {
  ...
  while (i <= text.length())
  {
    ...
    switch (state)
    {
      ...
      case Start:
        ...
        {
          i = text.length();
          setFormat(start, text.length(), m_colors[DLangEdit::Comment]);
        }
        else if (ch == '/' && next != '*')
        {
          ++i;
        ...
        else
        {
          if (!QString("(){}[]").contains(ch))
            setFormat(start, 1, m_colors[DLangEdit::Operator]);
          if (ch == '{' || ch == '}')
          {
            bracketPositions += i;
            if (ch == '{')
      ...
      case Number:
        ...
        {
          setFormat(start, i - start, m_colors[DLangEdit::Number]);
          state = Start;
        }
        else
        {
      ...
      case Identifier:
        if (ch.isSpace() || !(ch.isDigit() || ch.isLetter() || ch == '_'))
        {
          ...
          if (m_keywordsBlue.contains(token))
            setFormat(start, i - start, m_colors[DLangEdit::KeywordBlue]);
          if (m_keywordsPink.contains(token))
            setFormat(start, i - start, m_colors[DLangEdit::KeywordPink]);
          if (m_keywordsGreen.contains(token))
            setFormat(start, i - start, m_colors[DLangEdit::KeywordGreen]);
          else if (m_builtIn.contains(token))
            setFormat(start, i - start, m_colors[DLangEdit::BuiltIn]);
          state = Start;
        }
        else
        {
      ...
      case String:
        if (ch == text.at(start))
        {
          ...
          {
            ++i;
            setFormat(start, i - start, m_colors[DLangEdit::String]);
            state = Start;
          }
          else
          {
      ...
      case Comment:
        if (ch == '*' && next == '/')
        {
          ++i;
          ++i;
          setFormat(start, i - start, m_colors[DLangEdit::Comment]);
          state = Start;
        }
        else
        {
  ...
  {
    DLangBlockData* blockData = reinterpret_cast<DLangBlockData*>(currentBlock().userData());
    if (!blockData)
    {
      blockData = new DLangBlockData;
