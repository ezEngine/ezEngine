...
EZ_BEGIN_COMPONENT_TYPE(ezThirdPersonViewComponent, 1, ezComponentMode::Dynamic)
{
  EZ_BEGIN_PROPERTIES
  {
    ...
    EZ_MEMBER_PROPERTY("CeilingHeight", m_fCeilingHeight)->AddAttributes(new ezDefaultValueAttribute(4.0f)),
    EZ_MEMBER_PROPERTY("FloorHeight", m_fFloorHeight)->AddAttributes(new ezDefaultValueAttribute(0.0f)),
    EZ_MEMBER_PROPERTY("CollisionLayer", m_uiCollisionLayer)->AddAttributes(new ezDynamicEnumAttribute("PhysicsCollisionLayer")),
    EZ_MEMBER_PROPERTY("SweepWidth", m_fSweepWidth)->AddAttributes(new ezDefaultValueAttribute(0.2f), new ezClampValueAttribute(0.05f, 2.0f)),
...
void ezThirdPersonViewComponent::Update()
{
  ...
  float fNewDistance = m_fMaxDistance;

  if (m_CurUpRotation.GetRadian() > 0)
  {
    ezPlane p = ezPlane::MakeFromNormalAndPoint(-ezVec3::MakeAxisZ(), tTarget.m_vPosition + ezVec3::MakeAxisZ() * m_fCeilingHeight);

    float fHitDist;
    if (p.GetRayIntersection(vLookAtPos, vSweepDir, &fHitDist))
    {
      fNewDistance = ezMath::Min(fNewDistance, fHitDist);
    }
  }
  else
  {
    ezPlane p = ezPlane::MakeFromNormalAndPoint(ezVec3::MakeAxisZ(), tTarget.m_vPosition + ezVec3::MakeAxisZ() * m_fFloorHeight);
  ...
  if (ezPhysicsWorldModuleInterface* pPhysics = GetWorld()->GetOrCreateModule<ezPhysicsWorldModuleInterface>())
  {
    ...
    if (pPhysics->SweepTestSphere(res1, m_fSweepWidth, vLookAtPos, vSweepDir, fNewDistance, params))
    {
