...
static int CALLBACK MetaFileEnumProc(
  ...
  if (record->iType == EMR_EXTCREATEFONTINDIRECTW) {
    ...
    *reinterpret_cast<LOGFONTW*>(log_font) = create_font_record->elfw.elfLogFont;
  }
  return true;
}
...
static const std::vector<std::wstring>* GetLinkedFonts(const std::wstring& family) {
  ...
  HKEY key = NULL;
  if (FAILED(::RegOpenKeyExW(HKEY_LOCAL_MACHINE,
    kFontLinkKey,
    0,
    KEY_READ,
    &key))) {
  ...
  DWORD size = 0;
  RegQueryValueExW(key, family.c_str(), 0, NULL, NULL, &size);
  std::unique_ptr<BYTE[]> buffer(new BYTE[size]);
  if (SUCCEEDED(RegQueryValueExW(key,
    family.c_str(),
    0,
    NULL,
    buffer.get(),
...
LOGFONT GetFallbackFont(HDC dc, wchar_t character) {
  ...
  const std::vector<std::wstring>* linked_fonts = nullptr; /* EZ HACK! GetLinkedFonts(
...
String16 FontLoaderWin::fallback_font_for_characters(const String16& characters, int weight, bool italic, float size) const {
  ...
  if (logFont.lfFaceName[0])
    result = String16(logFont.lfFaceName, strlen(logFont.lfFaceName));
