...
ezUniquePtr<ezOpenXRGraphicsBinding> ezOpenXRGraphicsBinding::Create(ezGALDevice* pDevice)
{
  if (pDevice == nullptr)
  {
    ezLog::Error("OpenXR: Cannot create graphics binding - no GAL device available");
    return nullptr;
  }
  ...
  if (sRenderer == "Vulkan")
  {
    ...
    return EZ_DEFAULT_NEW(ezOpenXRGraphicsBindingVulkan);
  }
#endif
