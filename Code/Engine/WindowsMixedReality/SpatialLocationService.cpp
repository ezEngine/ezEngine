  ...
  , m_currentLocatability(ezSpatialLocatability::Unavailable)
{
  ...
  if (FAILED(m_pSpatialLocator->add_LocatabilityChanged(
        Callback<OnLocatabilityChangedFunc>(this, &ezWindowsSpatialLocationService::OnLocatabilityChanged).Get(),
        &m_eventRegistrationLocatabilityChanged)))
  {
...
ezUniquePtr<ezWindowsSpatialReferenceFrame> ezWindowsSpatialLocationService::CreateStationaryReferenceFrame_CurrentHeading(
  const ezWindowsSpatialReferenceFrame& origin, const ezTransform& offset)
{
  ...
  ComPtr<ISpatialStationaryFrameOfReference> pFrame;
  HRESULT result =
    m_pSpatialLocator->CreateStationaryFrameOfReferenceAtCurrentLocationWithPositionAndOrientation(vCurToDest, qCurToDest, pFrame.GetAddressOf());
  if (FAILED(result))
  {
    ezLog::Error("Failed to create stationary spatial reference frame at current position: {0}", ezHRESULTtoString(result));
    return nullptr;
...
ezUniquePtr<ezWindowsSpatialReferenceFrame> ezWindowsSpatialLocationService::CreateStationaryReferenceFrame(
  const ezWindowsSpatialReferenceFrame& origin, const ezTransform& offset)
{
  ComPtr<ISpatialCoordinateSystem> pOriginCoords;
  origin.GetInternalCoordinateSystem(pOriginCoords);
...
ezUniquePtr<ezWindowsSpatialReferenceFrame> ezWindowsSpatialLocationService::CreateStationaryReferenceFrame_Rotated(
...
ezUniquePtr<ezWindowsSpatialAnchor> ezWindowsSpatialLocationService::CreateSpatialAnchor(
  const ezTransform& offset, const ezWindowsSpatialReferenceFrame* pReferenceFrame /*= nullptr*/)
{
