...
void ezRcMarkPoiVisibleComponent::Update()
{
  ...
  for (ezUInt32 i = 0; i < points.GetCount(); ++i)
  {
    ...
    if (m_pPhysicsModule->CastRay(vOwnPos, vDirToBottom, fRayLenBottom, m_uiCollisionLayer, hit))
    {
      ...
      ezPhysicsHitResult hit;
      if (m_pPhysicsModule->CastRay(vOwnPos, vDirToTop, fRayLenTop, m_uiCollisionLayer, hit))
      {
