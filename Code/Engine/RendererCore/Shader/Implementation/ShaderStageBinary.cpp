...
void ezShaderConstantBufferLayout::Constant::CopyDataFormVariant(ezUInt8* pDest, ezVariant* pValue) const
{
  ...
  //ezLog::Error("Constant '{0}' is not set, invalid or couldn't be converted to target type and will be set to zero.", m_sName.GetData());
...
ezShaderStageBinary* ezShaderStageBinary::LoadStageBinary(ezGALShaderStage::Enum Stage, ezUInt32 uiHash)
{
  ...
  if (!itStage.IsValid())
  {
    ...
    {
      ezLog::Error("Could not read shader stage file '{0}'", sShaderStageFile.GetData());
      return nullptr;
    }
