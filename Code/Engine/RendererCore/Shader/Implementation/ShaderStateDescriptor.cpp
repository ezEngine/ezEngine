...
void ezShaderStateResourceDescriptor::Load(ezStreamReader& stream)
{
  ...
  EZ_ASSERT_DEV(uiVersion >= ezShaderStateVersion::Version1 && uiVersion <= ezShaderStateVersion::Current, "Invalid version {0}",
                uiVersion);
...
static ezInt32 GetEnumStateVariable(const ezMap<ezString, ezString>& variables, const ezMap<ezString, ezInt32>& values,
                                    const char* szVariable, ezInt32 defValue)
{
#if EZ_ENABLED(EZ_COMPILE_FOR_DEBUG)
  s_AllAllowedVariables.Insert(szVariable);
#endif
...
ezResult ezShaderStateResourceDescriptor::Load(const char* szSource)
{
  ...
  // Retrieve Blend State
  {
    ...
    for (ezUInt32 i = 0; i < 8; ++i)
    {
      ...
      m_BlendDesc.m_RenderTargetBlendDescriptions[i].m_BlendOpAlpha =
          (ezGALBlendOp::Enum)GetEnumStateVariable(VariableValues, StateValuesBlendOp, InsertNumber("BlendOpAlpha{0}", i, s),
                                                   m_BlendDesc.m_RenderTargetBlendDescriptions[0].m_BlendOpAlpha);
      ...
      m_BlendDesc.m_RenderTargetBlendDescriptions[i].m_DestBlendAlpha =
          (ezGALBlend::Enum)GetEnumStateVariable(VariableValues, StateValuesBlend, InsertNumber("DestBlendAlpha{0}", i, s),
                                                 m_BlendDesc.m_RenderTargetBlendDescriptions[0].m_DestBlendAlpha);
      m_BlendDesc.m_RenderTargetBlendDescriptions[i].m_SourceBlend =
          (ezGALBlend::Enum)GetEnumStateVariable(VariableValues, StateValuesBlend, InsertNumber("SourceBlend{0}", i, s),
                                                 m_BlendDesc.m_RenderTargetBlendDescriptions[0].m_SourceBlend);
      m_BlendDesc.m_RenderTargetBlendDescriptions[i].m_SourceBlendAlpha =
          (ezGALBlend::Enum)GetEnumStateVariable(VariableValues, StateValuesBlend, InsertNumber("SourceBlendAlpha{0}", i, s),
                                                 m_BlendDesc.m_RenderTargetBlendDescriptions[0].m_SourceBlendAlpha);
      m_BlendDesc.m_RenderTargetBlendDescriptions[i].m_uiWriteMask = GetIntStateVariable(
          VariableValues, InsertNumber("WriteMask{0}", i, s), m_BlendDesc.m_RenderTargetBlendDescriptions[0].m_uiWriteMask);
    }
  }
  ...
  // Retrieve Depth-Stencil State
  {
    ...
    m_DepthStencilDesc.m_DepthTestFunc = (ezGALCompareFunc::Enum)GetEnumStateVariable(VariableValues, StateValuesCompareFunc,
                                                                                      "DepthTestFunc", m_DepthStencilDesc.m_DepthTestFunc);
    ...
    m_DepthStencilDesc.m_uiStencilWriteMask =
        GetIntStateVariable(VariableValues, "StencilWriteMask", m_DepthStencilDesc.m_uiStencilWriteMask);
  }
