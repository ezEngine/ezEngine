...
/// Utilities for working with ezArchive files
namespace ezArchiveUtils
{
  ...
  /// Returns a modifiable array of file extensions that the engine considers to be valid ezArchive file extensions.
  ///
  ...
  /// Writes the header that identifies the ezArchive file and version to the stream
  EZ_FOUNDATION_DLL ezResult WriteHeader(ezStreamWriter& inout_stream);

  /// Reads the ezArchive header. Returns success and the version, if the stream is a valid ezArchive file.
  ...
  /// Writes a single file entry to an ezArchive stream with the given compression level.
  ///
