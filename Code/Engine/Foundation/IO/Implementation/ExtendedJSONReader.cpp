...
void ezExtendedJSONReader::OnEndObject()
{
  ...
  if (Child.m_Dictionary.TryGetValue("$t", Type))
  {
    ...
    {
      ezStringBuilder s;
      s.Format("The extended type variable '{0}' could not be converted. $t = '{1}', $v = '{2}', $b = '{3}'", Child.m_sName.GetData(), Type.ConvertTo<ezString>().GetData(), Value.ConvertTo<ezString>().GetData(), Binary.ConvertTo<ezString>().GetData());
    ...
    else
    {
      ...
      ezStringBuilder s;
      s.Format("Variable '{0}' is an extended type, but at the root of the document hierarchy. This is invalid.", Child.m_sName.GetData());
