...
protected:
  ...
  /// Must be implemented to create a ezDataDirectory_Reader for accessing the given file. Returns NULL if the file could not be opened.
  virtual ezDataDirectory_Reader* OpenFileToRead(const char* szFile) = 0;

  /// Must be implemented to create a ezDataDirectory_Writer for accessing the given file. Returns NULL if the file could not be opened. 
...
private:
  ...
  /// This is automatically called whenever a ezDataDirectory_ReaderWriter_Base that was opened by this type is being closed.
