...
namespace
{
  ...
  EZ_FORCE_INLINE ezSimdVec4f Lerp(const ezSimdVec4f& t, const ezSimdVec4f& a, const ezSimdVec4f& b)
  {
...
ezSimdVec4f ezSimdPerlinNoise::Noise(const ezSimdVec4f& inX, const ezSimdVec4f& inY, const ezSimdVec4f& inZ)
{
  ...
  return Lerp(w, Lerp(v, Lerp(u, Grad(Permute(AA), x, y, z), Grad(Permute(BA), x - f1, y, z)), Lerp(u, Grad(Permute(AB), x, y - f1, z), Grad(Permute(BB), x - f1, y - f1, z))),
    Lerp(v, Lerp(u, Grad(Permute(AA + i1), x, y, z - f1), Grad(Permute(BA + i1), x - f1, y, z - f1)),
      Lerp(u, Grad(Permute(AB + i1), x, y - f1, z - f1),
        Grad(Permute(BB + i1), x - f1, y - f1, z - f1))));
}
