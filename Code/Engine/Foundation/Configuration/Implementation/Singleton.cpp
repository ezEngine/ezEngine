...
void ezSingletonRegistry::Register(void* pSingletonInstance, const char* szTypeName)
{
  ...
  EZ_ASSERT_DEV(s_Singletons[szTypeName] == nullptr, "Singleton for type '%s' has already been registered", szTypeName);
...
{
  EZ_ASSERT_DEV(s_Singletons[szTypeName] != nullptr, "Singleton for type '%s' is currently not registered", szTypeName);
