...
namespace ezMemoryPolicies
{
  ...
  void* ezGuardedAllocation::Allocate(size_t uiSize, size_t uiAlign)
  {
    ...
    EZ_ASSERT_DEV(ptr != nullptr, "Could not commit memory pages. Error Code '%d'", ::GetLastError());
  ...
  void ezGuardedAllocation::Deallocate(void* ptr)
  {
    ...
    EZ_VERIFY(::VirtualFree(ptr, uiFullPageSize, MEM_DECOMMIT), "Could not decommit memory pages. Error Code '%d'", ::GetLastError());
