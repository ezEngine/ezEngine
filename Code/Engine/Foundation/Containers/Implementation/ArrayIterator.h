...
public:
  ...
  iterator_base& operator++() { m_iIndex += 1; return *this; }
  iterator_base& operator--() { m_iIndex -= 1; return *this; }

  iterator_base operator++(int) { m_iIndex += 1; return iterator_base(*m_Array, m_iIndex - 1); }
  iterator_base operator--(int) { m_iIndex -= 1; return iterator_base(*m_Array, m_iIndex + 1); }

  using const_iterator_base<ARRAY, T, reverse>::operator+;
  using const_iterator_base<ARRAY, T, reverse>::operator-;

  iterator_base operator+(ptrdiff_t rhs) const { return iterator_base(*m_Array, m_iIndex + rhs); }
  iterator_base operator-(ptrdiff_t rhs) const { return iterator_base(*m_Array, m_iIndex - rhs); }

  using const_iterator_base<ARRAY, T, reverse>::operator*;
  T& operator*() { if (reverse) return (*m_Array)[m_Array->GetCount() - (ezUInt32) m_iIndex - 1]; else return (*m_Array)[(ezUInt32) m_iIndex]; }
