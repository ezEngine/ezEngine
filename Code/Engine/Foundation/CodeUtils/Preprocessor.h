...
public:
  ...
  typedef ezDelegate<ezResult(const char* szCurAbsoluteFile, const char* szIncludeFile, IncludeType IncType, ezStringBuilder& out_sAbsoluteFilePath)>
    FileLocatorCB;
  ...
  ezResult Process(
    const char* szMainFile, ezStringBuilder& sOutput, bool bKeepComments = true, bool bRemoveRedundantWhitespace = false, bool bInsertLine = false);
...
private: // *** File Handling ***
  ...
  static ezResult DefaultFileLocator(
...
private: // *** Macro Definition ***
  ...
  ezResult StoreDefine(const ezToken* pMacroNameToken, const ezTokenParseUtils::TokenStream* pReplacementTokens, ezUInt32 uiFirstReplacementToken,
    ezInt32 iNumParameters, bool bUsesVarArgs);
...
private: // *** Parsing ***
  ezResult CopyTokensAndEvaluateDefined(
    const ezTokenParseUtils::TokenStream& Source, ezUInt32 uiFirstSourceToken, ezTokenParseUtils::TokenStream& Destination);
  void CopyTokensReplaceParams(const ezTokenParseUtils::TokenStream& Source, ezUInt32 uiFirstSourceToken, ezTokenParseUtils::TokenStream& Destination,
    const ezHybridArray<ezString, 16>& parameters);
  ...
  ezResult Expect(
    const ezTokenParseUtils::TokenStream& Tokens, ezUInt32& uiCurToken, const char* szToken1, const char* szToken2, ezUInt32* pAccepted = nullptr);
...
private: // *** Macro Expansion ***
  ...
  ezResult ExpandFunctionMacro(
    MacroDefinition& Macro, const MacroParameters& Parameters, ezTokenParseUtils::TokenStream& Output, const ezToken* pMacroToken);
  ...
  ezResult ExtractAllMacroParameters(
    const ezTokenParseUtils::TokenStream& Tokens, ezUInt32& uiCurToken, ezDeque<ezTokenParseUtils::TokenStream>& AllParameters);
  ...
  ezResult InsertStringifiedParameters(
    const ezTokenParseUtils::TokenStream& Tokens, ezTokenParseUtils::TokenStream& Output, const MacroDefinition& Macro);
...
private: // *** Other ***
  ...
  ezResult HandleInclude(
    const ezTokenParseUtils::TokenStream& Tokens, ezUInt32 uiCurToken, ezUInt32 uiDirectiveToken, ezTokenParseUtils::TokenStream& TokenOutput);
  ezResult HandleLine(
    const ezTokenParseUtils::TokenStream& Tokens, ezUInt32 uiCurToken, ezUInt32 uiDirectiveToken, ezTokenParseUtils::TokenStream& TokenOutput);
};
...
#define PP_LOG(Type, FormatStr, ErrorToken, ...)                                                                                                     \
  {                                                                                                                                                  \
    ...
    ezLog::Type(                                                                                                                                     \
      m_pLog, "File '{0}', Line {1} ({2}): {3}", _pe.m_pToken->m_File.GetString(), _pe.m_pToken->m_uiLine, _pe.m_pToken->m_uiColumn, sInfo);         \
  }
