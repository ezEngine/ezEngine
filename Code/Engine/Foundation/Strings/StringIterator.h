...
/// ezStringIterator allows for character-wise iterating over a (read-only) Utf8 string.
...
{
public:
  /// Creates a string iterator starting at the given position, ending at the next '\0' terminator.
  ...
  /// Creates a string iterator for the range from pFirst to pEnd, which currently points to pCurrent.
  /// If bIsPureASCII is set to true, a little more efficient iterating method is used.
  /// By default this flag should be kept false, as it will ensure that iterating will always work.
  ezStringIterator(const char* pFirst, const char* pEnd, const char* pCurrent, bool bIsPureASCII = false); // [tested]

  /// Advances the iterator to the next character, unless the end of the range was reached.
  void operator++(); // [tested]

  /// Moves the iteratore backwards to the previous character, unless the start of the range was reached.
  void operator--(); // [tested]
  ...
  /// Returns true, if the current iterator position is valid and will return a valid character.
  ...
  /// Returns the string, starting at the current iteration position.
  const char* GetData() const { return m_pCurrent; } // [tested]

  /// Returns the number of bytes from the current iteration position up to its end.
  ...
  /// Returns whether this sub-string represents a simple ASCII string, ie. whether the byte count and the character count will be identical.
  bool IsPureASCII() const { return m_bIsPureASCII; } // [tested]

  /// Returns the start of the iterator range.
  const char* GetStart() const { return m_pFirst; } // [tested]

  /// Returns the end of the iterator range. This will point to the byte AFTER the last character.
  ...
  /// Shrinks the iterator range by uiShrinkCharsFront characters at the front and by uiShrinkCharsBack characters at the back.
  ...
  /// Resets the current position to the first character and flags the iterator as valid again (unless its range is empty).
  void ResetToFront(); // [tested]

  /// Resets the current position to the last character and flags the iterator as valid again (unless its range is empty).
