...
public:
  ...
  const char* FindSubString(const char* szStringToFind) const; // [tested]

  /// Returns a pointer to the first occurrence of szStringToFind, or NULL if none was found. Case insensitive.
  /// To find the next occurrence, use an ezStringIterator which points to the next position and call FindSubString again.
  const char* FindSubString_NoCase(const char* szStringToFind) const; // [tested]
  ...
  const char* FindWholeWord(const char* szSearchFor, ezStringUtils::EZ_IS_WORD_DELIMITER IsDelimiterCB); // [tested]
  ...
  const char* FindWholeWord_NoCase(const char* szSearchFor, ezStringUtils::EZ_IS_WORD_DELIMITER IsDelimiterCB); // [tested]
