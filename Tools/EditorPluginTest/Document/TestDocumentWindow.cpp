...
static ezEditorEngineView g_EngineViewProcess;
ezEditorEngineView* ezEditorEngineView::s_pInstance = nullptr;



ezEditorEngineView::ezEditorEngineView()
{
  s_pInstance = this;
  m_iInitializedCount = 0;
}
...
void ezEditorEngineView::Initialize(ezUInt32 hWnd, ezUInt16 uiWidth, ezUInt16 uiHeight)
{
  ++m_iInitializedCount;

  if (m_iInitializedCount > 1)
    return;

  EZ_VERIFY(m_IPC.StartClientProcess("EditorEngineView.exe").Succeeded(), "Could not start engine view process");

  ezEngineViewMsg msg;
  msg.m_uiHWND = hWnd;
  msg.m_uiWindowWidth  = uiWidth;
  msg.m_uiWindowHeight = uiHeight;

  m_IPC.SendMessage(&msg);

  //m_pMemory = new QSharedMemory("ezEditorChannel");
  //EZ_VERIFY(m_pMemory->create(1024 * 1024) == true, "Shared memory already exists, probably a second editor instance has been opened.");

  // todo loop over names to find an unused channel

  //ezString sMsg = "Hello Process!";

  //m_pMemory->lock();
  //*((ezUInt32*) m_pMemory->data()) = hWnd;

  ////char* pData = (char*) m_pMemory->data();
  ////ezStringUtils::Copy((char*) pData, 1024 * 1024, sMsg.GetData());
  ////pData[sMsg.GetElementCount()] = '\0';
  //m_pMemory->unlock();


  //ezStringBuilder sPath = ezOSFile::GetApplicationDirectory();
  //sPath.AppendPath("EditorEngineView.exe");

  //m_pProcess = new QProcess;
  //m_pProcess->setProgram(sPath.GetData());

  //QStringList arguments;
  //arguments << "-channel";
  //arguments << "ezEditorChannel";

  //m_pProcess->setArguments(arguments);
  //m_pProcess->start();

  //EZ_VERIFY(m_pProcess->waitForStarted(), "Failed to run the external process");
}

void ezEditorEngineView::Deinitialize()
{
  --m_iInitializedCount;

  if (m_iInitializedCount > 0)
    return;
...
void ezEditorEngineView::Update()
{
...
ezTestDocumentWindow::~ezTestDocumentWindow()
{
  ...
  if (m_bInitializedProcess)
  {
    ezEditorEngineView::GetInstance()->Deinitialize();
  }
}
...
void ezTestDocumentWindow::InternalRedraw()
{
  if (!m_bInitializedProcess)
  {
    m_bInitializedProcess = true;
    ezEditorEngineView::GetInstance()->Initialize((ezUInt32) m_Window.GetNativeWindowHandle(), m_Window.GetClientAreaSize().width, m_Window.GetClientAreaSize().height);
  }
  else
  {
    if (!ezEditorEngineView::GetInstance()->m_IPC.IsClientAlive())
    {
      ezEditorEngineView::GetInstance()->Deinitialize();
      m_bInitializedProcess = false;
    }
    ...
    {
      ezEditorEngineView::GetInstance()->Update();
    }
  }
