...
EZ_CREATE_SIMPLE_TEST(Time, Timestamp)
{
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Constructors / Valid Check")
  {
    ...
    EZ_TEST(!invalidTimestamp.IsValid());

    ezTimestamp validTimestamp(0, ezSIUnitOfTime::Second);
    EZ_TEST(validTimestamp.IsValid());
    ...
    EZ_TEST(currentTimestamp.IsValid());
    EZ_TEST_MSG(currentTimestamp.GetInt64(ezSIUnitOfTime::Second) > 1384597970LL, "The current time is before this test was written!");
    EZ_TEST_MSG(currentTimestamp.GetInt64(ezSIUnitOfTime::Second) < 32531209845LL, "This current time is after the year 3000! If this is actually the case, please fix this test.");

    // Sleep for 10 milliseconds
    ezThreadUtils::Sleep(10);
    EZ_TEST_MSG(currentTimestamp.GetInt64(ezSIUnitOfTime::Microsecond) < ezTimestamp::CurrentTimestamp().GetInt64(ezSIUnitOfTime::Microsecond), "Sleeping for 10 ms should cause the timestamp to change!");
    EZ_TEST_MSG(!currentTimestamp.IsEqual(ezTimestamp::CurrentTimestamp(), ezTimestamp::CompareMode::Identical), "Sleeping for 10 ms should cause the timestamp to change!");
  ...
  {   
    ...
    EZ_TEST(epoch.IsValid());
    EZ_TEST(firstContact.IsValid());

    // GetInt64 / SetInt64
    ezTimestamp firstContactTest(iFirstContactUnixTimeInSeconds, ezSIUnitOfTime::Second);
    EZ_TEST(firstContactTest.GetInt64(ezSIUnitOfTime::Second) == iFirstContactUnixTimeInSeconds);
    EZ_TEST(firstContactTest.GetInt64(ezSIUnitOfTime::Millisecond) == iFirstContactUnixTimeInSeconds * 1000LL);
    EZ_TEST(firstContactTest.GetInt64(ezSIUnitOfTime::Microsecond) == iFirstContactUnixTimeInSeconds * 1000000LL);
    EZ_TEST(firstContactTest.GetInt64(ezSIUnitOfTime::Nanosecond) == iFirstContactUnixTimeInSeconds * 1000000000LL);

    firstContactTest.SetInt64(firstContactTest.GetInt64(ezSIUnitOfTime::Second), ezSIUnitOfTime::Second);
    EZ_TEST(firstContactTest.IsEqual(firstContact, ezTimestamp::CompareMode::Identical));
    ...
    EZ_TEST(firstContact.IsEqual(firstContactPlusAFewMicroseconds, ezTimestamp::CompareMode::FileTime));
    EZ_TEST(!firstContact.IsEqual(firstContactPlusAFewMicroseconds, ezTimestamp::CompareMode::Identical));
  }
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Operators")
  {
    ...
    EZ_TEST(timeSpan1000s.GetMicroseconds() == 1000000000LL);
    ...
    EZ_TEST(iSpanDiff == 1000000000LL);
    // You can only subtract points in time
    EZ_TEST(firstContactPlus1000s - firstContact == timeSpan1000s);
    ...
    EZ_TEST(iSpanDiff == 1000000000LL);
    // You can only subtract points in time
    EZ_TEST(T1000sPlusFirstContact - firstContact == timeSpan1000s);
    ...
    EZ_TEST(iSpanDiff == -1000000000LL);
    // You can only subtract points in time
    EZ_TEST(firstContact - firstContactMinus1000s == timeSpan1000s);


    // operator += / -=
    ezTimestamp testTimestamp = firstContact;
    testTimestamp += timeSpan1000s;
    EZ_TEST(testTimestamp.IsEqual(firstContactPlus1000s, ezTimestamp::CompareMode::Identical));
    testTimestamp -= timeSpan1000s;
    EZ_TEST(testTimestamp.IsEqual(firstContact, ezTimestamp::CompareMode::Identical));
  }
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "ezDateTime conversion")
  {
    ...
    EZ_TEST(!invalidDateTime.GetTimestamp().IsValid());
