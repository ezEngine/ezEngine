...
EZ_CREATE_SIMPLE_TEST(Math, Vec4)
{
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Constructor")
  {
    ...
    EZ_TEST(pDefCtor->x == (ezVec4T::ComponentType) 1 && 
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Length")
  {
    ...
    // IsZero
    EZ_TEST(ezVec4T::ZeroVector().IsZero());
    for (int i = 0; i < 4; ++i)
    {
    ...
    // IsZero(float)
    EZ_TEST(ezVec4T::ZeroVector().IsZero(0.0f));
    for (int i = 0; i < 4; ++i)
    {
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Operators")
  {
    ...
    vPlusAssign += vOp2;
    EZ_TEST(vPlusAssign.IsEqual(ezVec4T(-2.0f, 0.5f, -7.0f, 1.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // operator-= (ezVec4T)
    ezVec4T vMinusAssign = vOp1;
    vMinusAssign -= vOp2;
    EZ_TEST(vMinusAssign.IsEqual(ezVec4T(-6.0f, -0.1f, -7.0f, -1.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // operator*= (float)
    ezVec4T vMulFloat = vOp1;
    vMulFloat *= 2.0f;
    EZ_TEST(vMulFloat.IsEqual(ezVec4T(-8.0f, 0.4f, -14.0f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
    vMulFloat *= 0.0f;
    EZ_TEST(vMulFloat.IsEqual(ezVec4T::ZeroVector(), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // operator/= (float)
    ezVec4T vDivFloat = vOp1;
    vDivFloat /= 2.0f;
    EZ_TEST(vDivFloat.IsEqual(ezVec4T(-2.0f, 0.1f, -3.5f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    //operator+ (ezVec4T, ezVec4T)
    ezVec4T vPlus = (vOp1 + vOp2);
    EZ_TEST(vPlus.IsEqual(ezVec4T(-2.0f, 0.5f, -7.0f, 1.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    //operator- (ezVec4T, ezVec4T)
    ezVec4T vMinus = (vOp1 - vOp2);
    EZ_TEST(vMinus.IsEqual(ezVec4T(-6.0f, -0.1f, -7.0f, -1.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // operator* (float, ezVec4T)
    ezVec4T vMulFloatVec4 = ((ezMathTestType) 2 * vOp1);
    EZ_TEST(vMulFloatVec4.IsEqual(ezVec4T(-8.0f, 0.4f, -14.0f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
    vMulFloatVec4 = ((ezMathTestType) 0 * vOp1);
    EZ_TEST(vMulFloatVec4.IsEqual(ezVec4T::ZeroVector(), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // operator* (ezVec4T, float)
    ezVec4T vMulVec4Float = (vOp1 * (ezMathTestType) 2);
    EZ_TEST(vMulVec4Float.IsEqual(ezVec4T(-8.0f, 0.4f, -14.0f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
    vMulVec4Float = (vOp1 * (ezMathTestType) 0);
    EZ_TEST(vMulVec4Float.IsEqual(ezVec4T::ZeroVector(), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // operator/ (ezVec4T, float)
    ezVec4T vDivVec4Float = (vOp1 / (ezMathTestType) 2);
    EZ_TEST(vDivVec4Float.IsEqual(ezVec4T(-2.0f, 0.1f, -3.5f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Common")
  {
    ...
    // CompMin
    EZ_TEST(vOp1.CompMin(vOp2).IsEqual(ezVec4T(-4.0f, -0.3f, -7.0f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
    EZ_TEST(vOp2.CompMin(vOp1).IsEqual(ezVec4T(-4.0f, -0.3f, -7.0f, -0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // CompMax
    EZ_TEST(vOp1.CompMax(vOp2).IsEqual(ezVec4T(2.0f, 0.2f, 0.5f, 1.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
    EZ_TEST(vOp2.CompMax(vOp1).IsEqual(ezVec4T(2.0f, 0.2f, 0.5f, 1.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // CompMult
    EZ_TEST(vOp1.CompMult(vOp2).IsEqual(ezVec4T(-8.0f, -0.06f, -3.5f, 0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
    EZ_TEST(vOp2.CompMult(vOp1).IsEqual(ezVec4T(-8.0f, -0.06f, -3.5f, 0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));

    // CompDiv
    EZ_TEST(vOp1.CompDiv(vOp2).IsEqual(ezVec4T(-2.0f, -0.66666666f, -14.0f, 0.0f), ezMath::BasicType<ezMathTestType>::SmallEpsilon()));
  }
}