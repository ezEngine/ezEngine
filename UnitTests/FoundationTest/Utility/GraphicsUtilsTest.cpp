...
EZ_CREATE_SIMPLE_TEST(Utility, GraphicsUtils)
{
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Perspective (-1/1): ConvertWorldPosToScreenPos / ConvertScreenPosToWorldPos")
  {
    ...
    for (ezUInt32 y = 0; y < 25; ++y)
    {
      ...
      {
        ezVec3 vPoint, vDir;
        EZ_TEST_BOOL(ezGraphicsUtils::ConvertScreenPosToWorldPos(mProjInv, ezProjectionDepthRange::MinusOneToOne, 0, 0, 50, 25, ezVec3((float) x, (float) y, 0.5f), vPoint, &vDir).Succeeded());

        EZ_TEST_VEC3(vDir, vPoint.GetNormalized(), 0.01f);

        ezVec3 vScreen;
        EZ_TEST_BOOL(ezGraphicsUtils::ConvertWorldPosToScreenPos(mProj, ezProjectionDepthRange::MinusOneToOne, 0, 0, 50, 25, vPoint, vScreen).Succeeded());
  ...
  EZ_TEST_BLOCK(ezTestBlock::Enabled, "Perspective (0/1): ConvertWorldPosToScreenPos / ConvertScreenPosToWorldPos")
  {
    ...
    for (ezUInt32 y = 0; y < 25; ++y)
    {
      ...
      {
        ezVec3 vPoint, vDir;
        EZ_TEST_BOOL(ezGraphicsUtils::ConvertScreenPosToWorldPos(mProjInv, ezProjectionDepthRange::ZeroToOne, 0, 0, 50, 25, ezVec3((float) x, (float) y, 0.5f), vPoint, &vDir).Succeeded());

        EZ_TEST_VEC3(vDir, vPoint.GetNormalized(), 0.01f);

        ezVec3 vScreen;
        EZ_TEST_BOOL(ezGraphicsUtils::ConvertWorldPosToScreenPos(mProj, ezProjectionDepthRange::ZeroToOne, 0, 0, 50, 25, vPoint, vScreen).Succeeded());
